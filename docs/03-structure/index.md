---
title: "Phase 3: Structure"
sidebar_position: 3
description: "Clarity and containment."
phase: structure
---

# Structure

Naming patterns creates clarity. This phase translates that clarity into concrete structures — boundaries, routines, communication agreements, decision-making frameworks — that hold the new awareness in place.

**Coaching posture:** Values-to-structure translator.

**What this phase looks like:** Sessions connect a person's values to practical structures. We design boundaries, routines, and systems that feel both ambitious and doable. The work shifts from understanding to organizing.

**Moving on:** When a person wants to try their new structures in the real world — not just understand them — they're ready to practice.
